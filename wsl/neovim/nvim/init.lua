...
============================================================================================
  ...
  Cette configuration supporte deux environnements :
    1. VSCode Neovim Extension - Configuration minimale pour l'extension VSCode
    2. Neovim standalone - Configuration complète avec gestion des plugins

  Pour en savoir plus sur les différences entre ces modes :
    - Voir lua/options-vscode.lua pour les options VSCode
    - Voir lua/options.lua pour les options Neovim complètes
--]]

-- [[ Détection de l'Environnement ]]
--  Neovim peut fonctionner dans plusieurs environnements. La variable vim.g.vscode
--  est définie automatiquement par l'extension VSCode Neovim.
--
--  Si vous utilisez VSCode, seules les options compatibles sont chargées.
--  Sinon, la configuration complète avec tous les plugins est initialisée.
if vim.g.vscode then
    -- Mode VSCode : Configuration minimale
    print("VSCode extension")
    require("options-vscode")
else
    -- [[ Mode Neovim Standalone ]]
    ...
    -- stdpath("data") pointe vers le dossier de données de Neovim (~/.local/share/nvim sur Linux/Mac)
